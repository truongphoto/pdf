(() => {
  ...
  async function fetchNearbyPois(coords,radius){
    const q=`[out:json][timeout:9];(nwr["name"]["amenity"](around:${radius},${coords.lat},${coords.lng});nwr["name"]["healthcare"](around:${radius},${coords.lat},${coords.lng});nwr["name"]["shop"](around:${radius},${coords.lat},${coords.lng});nwr["name"]["tourism"](around:${radius},${coords.lat},${coords.lng});nwr["name"]["office"](around:${radius},${coords.lat},${coords.lng});nwr["name"]["public_transport"](around:${radius},${coords.lat},${coords.lng});nwr["name"]["place"](around:${radius},${coords.lat},${coords.lng}););out center tags 100;`;
    const eps=['https://overpass-api.de/api/interpreter','https://overpass.kumi.systems/api/interpreter'];
    let data=null;
    for(const ep of eps){
      try{const r=await fetchTimeout(ep,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:`data=${encodeURIComponent(q)}`},10000);if(r.ok){data=await r.json();break}}catch(_){}
    }
    if(!data||!Array.isArray(data.elements))return[];
    const out=[];
    for(const e of data.elements){
      const t=e.tags||{},name=String(t.name||'').trim();if(!name)continue;
      const la=Number(e.lat??(e.center&&e.center.lat)),lo=Number(e.lon??(e.center&&e.center.lon));if(!Number.isFinite(la)||!Number.isFinite(lo))continue;
      const cat=t.healthcare||t.amenity||t.shop||t.tourism||t.office||t.public_transport||t.place||'poi';
      const pri=(t.healthcare||['pharmacy','hospital','clinic','doctors'].includes(t.amenity))?0:(['school','college','university','kindergarten'].includes(t.amenity)?1:(t.public_transport?2:(t.place?3:(t.shop?4:5))));
      out.push({name,lat:la,lng:lo,cat,pri,d:haversine(coords,{lat:la,lng:lo})});
    }
    out.sort((a,b)=>a.pri-b.pri||a.d-b.d);
    const seen=new Set(),res=[];for(const x of out){const k=x.name.toLowerCase();if(seen.has(k))continue;seen.add(k);res.push(x);if(res.length>=24)break}return res;
  }
  ...
  function drawPois(ctx,pois,z,left,top,scale,cw,ch){
    if(!pois||!pois.length)return;const boxes=[];ctx.save();ctx.font=`600 ${10.5*scale}px Arial`;ctx.textBaseline='middle';let count=0;
    for(const p of pois){const wp=mercator(p.lat,p.lng,z),px=(wp.x-left)*scale,py=(wp.y-top)*scale;if(px<20*scale||py<18*scale||px>cw-20*scale||py>ch-28*scale)continue;
      const name=p.name.length>31?p.name.slice(0,30)+'…':p.name,tw=ctx.measureText(name).width,h=19*scale,w=tw+20*scale;let bx=px+6*scale,by=py-h/2;if(bx+w>cw-6*scale)bx=px-w-6*scale;
      const r={x:bx,y:by,w,h};if(boxes.some(b=>!(r.x+r.w<b.x||b.x+b.w<r.x||r.y+r.h<b.y||b.y+b.h<r.y)))continue;boxes.push(r);
      const health=/pharmacy|hospital|clinic|doctors|health/.test(String(p.cat)),shop=p.pri===4;const dot=health?'#1689df':(shop?'#e58b31':'#6f8796'),text=health?'#0d65a7':(shop?'#a45c17':'#4f6572');
      ctx.fillStyle='rgba(255,255,255,.93)';ctx.strokeStyle='rgba(50,70,80,.22)';ctx.lineWidth=1*scale;roundedRect(ctx,bx,by,w,h,7*scale);ctx.fill();ctx.stroke();ctx.beginPath();ctx.arc(bx+7*scale,by+h/2,3*scale,0,Math.PI*2);ctx.fillStyle=dot;ctx.fill();ctx.fillStyle=text;ctx.fillText(name,bx+13*scale,by+h/2+.2*scale);if(++count>=16)break;
    }ctx.restore();
  }
  ...
  async function buildMap(coords){
    ...
    let ok=0,done=0;await Promise.all(jobs.map(async j=>{try{const im=await tileWithFallback(z,j.x,j.y);x.drawImage(im,j.dx,j.dy,512,512);if(im&&typeof im.close==='function')try{im.close()}catch(_){};ok++}catch(_){}done++;setProgress(18+Math.round(done/Math.max(1,jobs.length)*32),'Đang dựng nền bản đồ…')}));
    if(!ok) drawFallbackGrid(x,c.width,c.height,coords);
    if(ok){
      setProgress(52,'Đang bổ sung địa điểm nhận biết…');
      try{
        const mpp=156543.03392*Math.cos(coords.lat*Math.PI/180)/(2**z),radius=Math.max(250,Math.min(1200,Math.round(Math.max(lw,lh)*mpp*.62)));
        const pois=await fetchNearbyPois(coords,radius);drawPois(x,pois,z,left,top,scale,c.width,c.height);
      }catch(_){}
    }
  ...
  if(previewSync){previewSync.title='v26: Preview Google Maps ổn định; PDF chạy ngầm bằng OpenStreetMap + địa điểm nhận biết';previewSync.classList.add('v26-ready');previewSync.textContent='Google Maps sẵn sàng'}
})();
